...
function svg2gcode(userconfig, svgfile, callback){

  console.log(userconfig);

  getPoints(svgfile, function(data){
    //pathSort(data, function(data){
      generateGcode(userconfig, data, function(gcode){
        callback(gcode);
      });
    //});
  });
...
function penUp() {
   var parts = ['M106 S220', 'G4 P35'];
   gcode.push(parts.join('\n'));
}

function penDown(wipe) {
   var parts = ['M106 S170', 'G4 P35'];
   gcode.push(parts.join('\n'));
}


function endgcode() {
   var parts = ['M106 S220', 'G4 P10', 'G1 X100 Y100', 'G28 Y0', 'G28 X0'];
   //var parts = [];
   gcode.push(parts.join('\n'));
}
...
function generateGcode(config, paths, callback){
  
  gcode = ["G21","G90","M106 S220","G4 P10","G28 Y0","G28 X0","G1 F30000"];
  ...
  for (var i = 0, len = paths.length; i < len; i++) {
      ...
      while(sub--){
          ...
          move({ x : (((paths[i].points[sub].x))).toFixed(5), y: (65+config.plotterHeight - ((paths[i].points[sub].y))).toFixed(5)});
          if(sub == 0){
             penUp();
          }
      }
